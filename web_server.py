...
class AgentStateManager:
    ...
    def __init__(self):
        ...
        self._last_screenshot_hash: Optional[str] = (
            None  # 上次截图的哈希值（用于差分检测）
        )
    ...
    def add_log(self, message: str, level: str = "info", details: str = None):
        ...
        # 广播日志
        asyncio.create_task(self.broadcast({"type": "log", "payload": log_entry}))
    ...
    async def broadcast_state(self):
        """广播当前状态"""
        await self.broadcast({"type": "state_update", "payload": self.get_state_dict()})
    ...
    async def add_terminal_line(self, line: str, line_type: str = "output"):
        ...
        await self.broadcast({"type": "terminal_line", "payload": line_entry})
    ...
    async def set_waiting_for_input(self, prompt: str = ""):
        ...
        await self.broadcast(
            {"type": "input_required", "payload": {"waiting": True, "prompt": prompt}}
        )
    ...
    async def submit_user_input(self, user_input: str, is_password: bool = False):
        ...
        await self.broadcast(
            {"type": "input_required", "payload": {"waiting": False, "prompt": ""}}
        )
    ...
    async def launch_browser(self) -> bool:
        ...
        try:
            ...
            self.add_log(
                f"Trying to connect to agent browser via CDP: {cdp_url}", "info"
            )
            ...
            try:
                ...
                if contexts:
                    ...
                    if pages:
                        ...
                        self.page = pages[-1]
                        self.add_log(
                            f"Using latest page: {self.page.url[:50] if self.page.url else 'about:blank'}",
                            "info",
                        )
                    else:
                ...
                self.add_log(
                    f"CDP connection failed: {str(cdp_error)[:100]}", "warning"
                )
    ...
    def _setup_browser_listeners(self):
        ...
        def on_context_created(context):
            ...
            def on_page_created(page):
                print(
                    f"[Screenshot Stream] New page created: {page.url if page.url else 'about:blank'}"
                )
                self.page = page
                self.add_log(
                    f"Switched to new page: {page.url[:50] if page.url else 'about:blank'}",
                    "info",
                )
            ...
            if pages:
                self.page = pages[-1]
                print(
                    f"[Screenshot Stream] Using existing page: {self.page.url if self.page.url else 'about:blank'}"
                )
        ...
        try:
            ...
            for ctx in self.browser.contexts:
                ...
                def on_page_created_for_ctx(page, ctx=ctx):
                    print(
                        f"[Screenshot Stream] New page in existing context: {page.url if page.url else 'about:blank'}"
                    )
                    self.page = page
                    self.context = ctx
                    self.add_log(
    ...
    async def start_screenshot_stream(self):
        ...
        self.add_log(
            f"Screenshot stream started (target: {SCREENSHOT_TARGET_FPS}fps)", "info"
        )
                    ...
                    if contexts:
                        ...
                        if latest_context.pages:
                            ...
                            if latest_page != self.page:
                                print(
                                    f"[Screenshot Loop] Switching to newer page: {latest_page.url if latest_page.url else 'about:blank'}"
                                )
                ...
                if self.page:
                    try:
                        ...
                        if screenshot_hash != self._last_screenshot_hash:
                            ...
                            # 广播截图
                            await self.broadcast_screenshot(
                            ...
                            if self.browser and self.browser.contexts:
                                for ctx in reversed(self.browser.contexts):
                                    if ctx.pages:
                                        ...
                                        self.context = ctx
                                        print(
                                            f"[Screenshot Loop] Switched to backup page"
                                        )
                                        break
    ...
    async def execute_main_py(
        ...
        if max_steps:
            ...
            # 使用 main.py 的默认值
            self.max_steps = 30
        if model and model in get_available_model_catalog(include_secrets=False):
    ...
    async def _delayed_start_screenshot_stream(
        self, max_retries: int = 15, retry_delay: float = 1.0
    ):
        """
        延迟启动截图流
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                cdp_url = f"http://{CDP_HOST}:{CDP_PORT}"
                print(
                    f"[Screenshot Stream] Attempt {attempt + 1}/{max_retries}: Connecting to {cdp_url}"
                )
                self.browser = await self.playwright.chromium.connect_over_cdp(cdp_url)
                ...
                if contexts:
                    ...
                    pages = self.context.pages
                    print(
                        f"[Screenshot Stream] Found {len(pages)} pages in last context"
                    )
                    ...
                    if pages:
                        ...
                        self.add_log(
                            f"Connected to agent browser: {page_url[:50]}", "success"
                        )
                        print(
                            f"[Screenshot Stream] Connected to latest page: {page_url}"
                        )

                        # 启动截图流
                        await self.start_screenshot_stream()
                        print(
                            f"[Screenshot Stream] Screenshot stream started successfully"
                        )
                        return
                    else:
                ...
                print(
                    f"[Screenshot Stream] Connection attempt {attempt + 1} failed: {e}"
                )
                if attempt < max_retries - 1:
                    self.add_log(
                        f"Waiting for agent browser... (attempt {attempt + 1}/{max_retries})",
                        "info",
                    )
                else:
                    self.add_log(
                        f"Failed to connect to agent browser after {max_retries} attempts",
                        "warning",
                    )
    ...
    async def _read_command_output(self):
        ...
        try:
            while True:
                try:
                    line = await asyncio.wait_for(
                        self.command_process.stdout.readline(), timeout=0.5
                    )
                    ...
                    if line_text:
                        ...
                        if progress_match:
                            ...
                            if new_progress != self.progress_ratio:
                                ...
                                print(
                                    f"[WebSocket] Progress updated to {new_progress:.0%}"
                                )
                        ...
                        complexity_match = re.search(
                            r"任务复杂度更新:\s*(simple|medium|complex|very_complex)",
                            line_text,
                        )
                        if complexity_match:
                            ...
                            if new_complexity != self.task_complexity:
                                ...
                                print(
                                    f"[WebSocket] Task complexity updated to {new_complexity}"
                                )

                        # 广播状态更新（每次输出都广播，确保前端实时显示）
                        # 打印当前状态用于调试
                        print(
                            f"[WebSocket] Broadcasting state: step={self.current_step}, stepDesc={self.step_description[:50]}, lastAction={self.last_action[:30]}"
                        )
                    ...
                    try:
                        ...
                        if chunk:
                            ...
                            if chunk_stripped:
                                ...
                                # 解析进度信息
                                clean_chunk = re.sub(
                                    r"\x1b\[[0-9;]*m", "", chunk_stripped
                                )
                                progress_match = re.search(
                                    r"📊 进度:\s*(\d+)%", clean_chunk
                                )
                                if progress_match:
                                    new_progress = int(progress_match.group(1)) / 100
                                    if new_progress != self.progress_ratio:
            ...
            self.command_status = (
                "completed" if self.command_exit_code == 0 else "error"
            )
            ...
            await self.add_terminal_line(
                f"[Process finished with exit code: {self.command_exit_code}]", "system"
            )
            self.add_log(
    ...
    def _detect_line_type(self, line: str) -> str:
        ...
        # 检测输入提示
            ...
            kw in line_lower for kw in ["password", "密码", "input", "enter", "请输入"]
        ):
            return "prompt"
        else:
            return "output"
    ...
    async def _handle_input_request(self, prompt_line: str):
        ...
        """
        self._current_input_is_password = any(
            kw in prompt_line.lower() for kw in ["password", "密码", "主密码"]
        )
    ...
    def _parse_output_for_status(self, line: str) -> bool:
        ...
        if "正在启动浏览器" in clean_line or "launching browser" in clean_line.lower():
            ...
            "错误" in clean_line or "error" in clean_line.lower()
        ):
            self.status = "error"
        ...
        await self.broadcast(
            {
                ...
                "payload": {
                    ...
                    "duration": datetime.now().timestamp() - self.command_start_time
...
llm_config_store = get_llm_config_store()
...
async def get_models():
    """获取可用模型列表"""
    models = []
    for model_id, config in get_available_model_catalog(include_secrets=False).items():
        models.append(
            {
                "id": model_id,
                "name": config["name"],
                ...
                "provider": config.get("provider", "builtin"),
            }
        )
    return models
...
async def create_llm_config(request: LLMConfigRequest):
    ...
    if not request.base_url.strip().startswith(("http://", "https://")):
        raise HTTPException(
    ...
    created = llm_config_store.create_config(
...
async def update_llm_config(config_id: str, request: LLMConfigUpdateRequest):
    if not llm_config_store.get_config(config_id):
        raise HTTPException(status_code=404, detail="配置不存在")

    updates = request.model_dump(exclude_unset=True)
    if (
        "base_url" in updates
        and updates["base_url"]
        and not updates["base_url"].startswith(("http://", "https://"))
    ):
        raise HTTPException(
            status_code=400, detail="base_url 必须以 http:// 或 https:// 开头"
        )

    updated = llm_config_store.update_config(config_id, **updates)
    return {"success": True, "config": updated}


@app.delete("/api/llm-configs/{config_id}")
async def delete_llm_config(config_id: str):
    if not llm_config_store.delete_config(config_id):
        raise HTTPException(status_code=404, detail="配置不存在")
...
async def start_agent(request: StartAgentRequest):
    ...
    try:
        ...
        requested_model = request.model or llm_config_store.get_default_model_id() or ""
        if not requested_model:
            return JSONResponse(
                status_code=400,
                content={
                    "success": False,
                    "message": "当前没有可用的自定义模型配置，请先在 API CONFIG 中添加并启用配置",
                },
            )

        model_config = get_model_config(requested_model, include_secret=True)
        if (
            not model_config
            or not model_config.get("api_key")
            or not model_config.get("api_base")
        ):
            return JSONResponse(
                status_code=400,
                content={
                    "success": False,
                    "message": "当前模型缺少完整的自定义 API 配置，请在 API CONFIG 中补全 API Key 与 Base URL",
                },
            )

        # 保存 objective
        agent_state.objective = request.objective
        agent_state.selected_model = requested_model
        ...
        agent_state.add_log(
            f"Agent started with model: {requested_model}", "success", request.objective
        )
        ...
        await agent_state.broadcast(
            {"type": "status_change", "payload": {"status": "running"}}
        )
        ...
        if browser_launched:
            ...
            agent_state.add_log(
                "Browser launch failed, continuing without browser", "warning"
            )
        ...
        return {
            "success": True,
            "message": f"Agent started with objective: {request.objective}",
        }
...
async def _async_start_agent():
    ...
    try:
        ...
        if browser_launched:
            ...
            agent_state.add_log(
                "Browser launch returned False, continuing without browser", "warning"
            )
...
async def pause_agent():
    ...
    await agent_state.broadcast_state()
    await agent_state.broadcast(
        {"type": "status_change", "payload": {"status": "paused"}}
    )
...
async def stop_agent():
    ...
    await agent_state.broadcast_state()
    await agent_state.broadcast(
        {"type": "status_change", "payload": {"status": "stopped"}}
    )
...
async def reset_agent():
    ...
    await agent_state.broadcast_state()
    await agent_state.broadcast(
        {"type": "status_change", "payload": {"status": "idle"}}
    )
...
async def get_terminal_output(limit: int = 100):
    """获取终端输出"""
    return {
        "lines": agent_state.terminal_lines[-limit:]
        if agent_state.terminal_lines
        else [],
...
async def clear_terminal():
    ...
    await agent_state.broadcast({"type": "terminal_cleared", "payload": {}})
...
async def stop_command():
    ...
    return {
        "success": success,
        "message": "Command stopped" if success else "Failed to stop command",
    }
...
async def get_command_status():
    ...
    return {
        ...
        "duration": datetime.now().timestamp() - agent_state.command_start_time
        if agent_state.command_start_time
        else 0,
        "output": agent_state.command_output[-50:]
        if agent_state.command_output
        else [],
    }
...
async def get_command_output(limit: int = 100):
    """获取命令输出"""
    return {
        "output": agent_state.command_output[-limit:]
        if agent_state.command_output
        else [],
...
async def get_file_content(file_path: str):
    ...
    try:
        ...
        if not any(
            resolved_path.startswith(prefix.lower()) for prefix in allowed_prefixes
        ):
...
async def get_all_files():
    ...
    if LOGS_DIR.exists():
        for file_path in LOGS_DIR.iterdir():
            if file_path.is_file():
                ...
                if original_task_id:
                    # 使用分组后的代表task_id
                    grouped_task_id = task_id_mapping.get(
    ...
    sorted_groups = sorted(
        task_groups.values(), key=lambda x: x["task_id"], reverse=True
    )
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        await websocket.send_json(
            {"type": "state_update", "payload": agent_state.get_state_dict()}
        )

        # 发送连接成功消息
        await websocket.send_json(
            {"type": "connection_status", "payload": {"connected": True}}
        )
        ...
        if agent_state.status == "running" and agent_state.command_status != "running":
            ...
            actions = [
                (
                    ...
                    lambda: agent_state.navigate_to("https://www.baidu.com"),
                ),
                ...
                (
                    "Typing search query...",
                    lambda: agent_state.type_text(agent_state.objective),
                ),
                ...
                (
                    "Scrolling down page...",
                    lambda: (
                        agent_state.page.evaluate("window.scrollBy(0, 300)")
                        if agent_state.page
                        else asyncio.sleep(0.5)
                    ),
                ),
            ...
            if agent_state.current_step <= len(actions):
                ...
                try:
                    ...
                    agent_state.step_description = (
                        f"[Step {agent_state.current_step}] {action_name}"
                    )
