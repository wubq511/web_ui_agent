...
class ModelManager:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        default_model_id = get_default_model_id() or ""
        self._current_model: str = default_model_id
        self._initial_model: str = default_model_id
        self._switch_history: list[SwitchRecord] = []
        self._last_switch_time: float = 0
        self._config = MODEL_SWITCH_CONFIG

        for model_id in self._get_available_models():
            self._model_stats[model_id] = ModelStats()

        if default_model_id:
            self._record_switch(
                from_model="none",
                to_model=default_model_id,
    ...
    def _get_available_models(
        ...
        return get_available_model_catalog(include_secrets=include_secrets)

    def _ensure_model_stats(self, model_id: str):
        if model_id not in self._model_stats:
            self._model_stats[model_id] = ModelStats()

    def _get_auto_switch_target_model(self) -> Optional[str]:
        available_models = self._get_available_models()

        preferred_target = available_models.get(AUTO_SWITCH_TARGET_MODEL)
        if preferred_target and preferred_target.get("supports_auto_switch", False):
            return AUTO_SWITCH_TARGET_MODEL

        for model_id, config in available_models.items():
            if model_id == self._current_model:
                continue
            if config.get("supports_auto_switch", False):
                return model_id
    ...
    def set_initial_model(self, model_id: str) -> bool:
        ...
        bool: 是否设置成功
        """
        if model_id not in self._get_available_models():
            return False
    ...
    def _create_llm_instance(self, model_id: str) -> ChatOpenAI:
        ...
        ChatOpenAI: 模型实例
        """
        model_config = get_model_config(model_id, include_secret=True)
        if not model_config:
            raise ValueError(f"未知模型: {model_id}")

        runtime_model = model_config.get("runtime_model", model_id)
        model_api_key = model_config.get("api_key")
        model_api_base = model_config.get("api_base")

        if not model_api_key:
            raise ValueError(f"模型 {model_id} 缺少可用的 API Key 配置")
        if not model_api_base:
            raise ValueError(f"模型 {model_id} 缺少可用的 Base URL 配置")
    ...
    def get_model_info(self, model_id: str) -> Optional[Dict[str, Any]]:
        ...
        dict: 模型配置信息
        """
        return self._get_available_models(include_secrets=True).get(model_id)
    ...
    def get_model_stats(self, model_id: str) -> ModelStats:
        ...
        ModelStats: 模型统计信息
        """
        self._ensure_model_stats(model_id)
    ...
    def record_failure(self, model_id: str) -> bool:
        ...
        if stats.consecutive_failures >= threshold and self._config.get(
    ...
    def _should_switch_back(self, current_model: str) -> bool:
        ...
        target_model = self._get_auto_switch_target_model()
        if not target_model:
            return False
    ...
    def _auto_switch_back(self):
        ...
        print(
            f"🔄 模型自动切换: {from_model} -> {target_model} (连续成功，切回初始模型)"
        )
    ...
    def _auto_switch_on_failure(self, failed_model: str) -> bool:
        ...
        self._record_switch(
            ...
            detail="连续失败，自动切换到备用模型",
        )

        print(
            f"🔄 模型自动切换: {from_model} -> {target_model} (连续失败，切换到备用模型)"
        )
        return True

    def switch_on_stagnation(
        self, stagnation_count: int, stagnation_threshold: int
    ) -> bool:
        """
        因任务停滞自动切换模型
        ...
        self._record_switch(
            ...
            detail=f"任务停滞({stagnation_count}/{stagnation_threshold})，切换到备用模型",
        )

        print(
            f"🔄 模型自动切换: {from_model} -> {target_model} (任务停滞 {stagnation_count}/{stagnation_threshold}，切换到备用模型)"
        )
        return True
    ...
    def _record_switch(
        self, from_model: str, to_model: str, reason: SwitchReason, detail: str = ""
    ):
        """
        记录模型切换
    ...
    def get_status_display(self) -> str:
        ...
        lines.append(f"\n默认模型: {self._initial_model or '未配置'}")
        lines.append(f"初始模型: {self._initial_model}")
        lines.append(f"自动切换目标: {AUTO_SWITCH_TARGET_MODEL} (若存在且已配置)")
        ...
        for model_id, config in available_models.items():
            ...
            auto_tag = (
                "[自动]" if config.get("supports_auto_switch", False) else "[手动]"
            )
    ...
    def list_models(self) -> str:
        ...
        for model_id, config in available_models.items():
            self._ensure_model_stats(model_id)
            is_current = "👉 " if model_id == self._current_model else "   "
            is_default = " [默认]" if model_id == self._initial_model else ""
            is_auto = (
                " [支持自动切换]"
                if config.get("supports_auto_switch", False)
                else " [仅手动切换]"
...
def init_model_manager(api_key: Optional[str] = None) -> ModelManager:
    """
    初始化模型管理器单例
